...
const getFinancialSummarySchema = z.object({
  ...
  groupBy: z
    .enum(['day', 'week', 'month'])
    .default('day')
    .describe('Group results by period'),
});
...
const getInventoryReportSchema = z.object({
  includeValuation: z
    .boolean()
    .default(true)
    .describe('Include inventory valuation'),
  warehouse: z.string().optional().describe('Filter by specific warehouse'),
  lowStockOnly: z
    .boolean()
    .default(false)
    .describe('Only show items below reorder point'),
});
...
const getEmployeeReportSchema = z.object({
  ...
  includePayroll: z
    .boolean()
    .default(false)
    .describe('Include payroll information'),
  activeOnly: z
    .boolean()
    .default(true)
    .describe('Only include active employees'),
});
...
function generateHistoricalSummaries(startDate, endDate) {
  ...
  while (currentDate <= endDate) {
    ...
    const expenseRatio = faker.number.float({
      min: 0.6,
      max: 0.8,
      fractionDigits: 2,
    });
    ...
    const summary = {
      ...
      topProducts: getTopProducts(3).map((p) => ({
        ...
        revenue:
          p.revenue *
          faker.number.float({ min: 0.05, max: 0.15, fractionDigits: 2 }),
...
export const analyticsTools = [
  {
    ...
    inputSchema: z.toJSONSchema(getFinancialSummarySchema, {
      target: 'draft-7',
      io: 'input',
    }),
  },
  {
    ...
    inputSchema: z.toJSONSchema(getInventoryReportSchema, {
      target: 'draft-7',
      io: 'input',
    }),
  },
  {
    ...
    inputSchema: z.toJSONSchema(getSalesReportSchema, {
      target: 'draft-7',
      io: 'input',
    }),
  },
  {
    ...
    inputSchema: z.toJSONSchema(getEmployeeReportSchema, {
      target: 'draft-7',
      io: 'input',
    }),
  },
  {
    ...
    inputSchema: z.toJSONSchema(getKPIMetricsSchema, {
      target: 'draft-7',
      io: 'input',
    }),
  },
  ...
  switch (name) {
    case 'get_financial_summary': {
      ...
      if (input.groupBy === 'week') {
        ...
        groupedSummaries = Array.from(weeklyMap.entries()).map(
          ([weekStart, summaries]) => ({
            date: weekStart,
      ...
      const totalRevenue = groupedSummaries.reduce(
        (sum, s) => sum + s.revenue,
        0,
      );
      const totalExpenses = groupedSummaries.reduce(
        (sum, s) => sum + s.expenses,
        0,
      );
      const totalProfit = groupedSummaries.reduce(
        (sum, s) => sum + s.profit,
        0,
      );
      const totalOrders = groupedSummaries.reduce(
      ...
      return {
        ...
        trends: {
          revenueGrowth: faker.number.float({
            min: -5,
            max: 15,
            fractionDigits: 2,
          }),
          orderGrowth: faker.number.float({
            min: -3,
            max: 12,
            fractionDigits: 2,
          }),
          profitGrowth: faker.number.float({
            min: -8,
            max: 20,
            fractionDigits: 2,
          }),
    ...
    case 'get_inventory_report': {
      ...
      if (input.warehouse) {
        inventoryItems = inventoryItems.filter(
          (item) => item.warehouse === input.warehouse,
        );
      }
      ...
      const totalValue = enrichedInventory.reduce(
        (sum, item) => sum + item.stockValue,
        0,
      );
      const itemsBelowReorderPoint = enrichedInventory.filter(
      ...
      return {
        ...
        summary: {
          ...
          totalReorderCost: input.includeValuation
            ? totalReorderCost
            : undefined,
          warehouses: [
            ...new Set(enrichedInventory.map((item) => item.warehouse)),
          ],
        ...
        case 'category': {
          ...
          reportData = Array.from(categorySales.entries())
            .map(([category, data]) => ({
              ...
              orderCount: data.orderCount,
              averageOrderValue: data.revenue / data.orderCount,
              productCount: mockProducts.filter((p) => p.category === category)
                .length,
            }))
            .sort((a, b) => b.revenue - a.revenue);
          break;
    ...
    case 'get_employee_report': {
      ...
      return {
        employees: employees.map((emp) => ({
          ...
          yearsOfService:
            new Date().getFullYear() - new Date(emp.hireDate).getFullYear(),
        })),
        summary: {
          ...
          byStatus: {
            ...
            terminated: employees.filter((e) => e.status === 'terminated')
              .length,
          },
          byDepartment: Object.entries(departmentStats).map(
            ([dept, stats]) => ({
              department: dept,
              employeeCount: stats.count,
              totalPayroll: input.includePayroll
                ? stats.totalSalary
                : undefined,
    ...
    case 'get_kpi_metrics': {
      ...
      const orderCount = periodOrders.filter(
      ...
      const repeatCustomers = periodOrders.reduce((acc, order) => {
        const customerOrders = periodOrders.filter(
      ...
      // Inventory metrics
      const stockTurnover = faker.number.float({
        min: 2,
        max: 8,
        fractionDigits: 2,
      });
      ...
      return {
        ...
        customerMetrics: {
          ...
          customerLifetimeValue: faker.number.float({
            min: 5000,
            max: 25000,
            fractionDigits: 2,
          }),
        ...
        inventoryMetrics: {
          ...
          itemsBelowReorderPoint: mockInventory.filter(
        ...
        operationalMetrics: {
          ...
          averageShippingTime:
            faker.number.float({ min: 1.5, max: 3.5, fractionDigits: 1 }) +
            ' days',
        ...
        employeeMetrics: {
          ...
          employeeSatisfactionScore:
            faker.number.float({ min: 3.5, max: 4.8, fractionDigits: 1 }) +
            '/5',
        },
        financialHealth: {
          profitMargin: `${faker.number.float({ min: 15, max: 35, fractionDigits: 2 })}%`,
          cashFlowStatus: faker.helpers.arrayElement([
            'positive',
            'stable',
            'improving',
          ]),
          debtToEquityRatio: faker.number.float({
            min: 0.2,
            max: 0.8,
            fractionDigits: 2,
          }),
          workingCapitalRatio: faker.number.float({
            min: 1.2,
            max: 2.5,
            fractionDigits: 2,
          }),
