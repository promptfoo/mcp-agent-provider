...
const createProductSchema = z.object({
  ...
  unitOfMeasure: z
    .string()
    .default('unit')
    .describe('Unit of measure (unit, kg, meter, etc.)'),
});
...
const createCustomerSchema = z.object({
  ...
  paymentTerms: z
    .enum(['net30', 'net60', 'net90', 'prepaid'])
...
function generateMockOrders(customerId, count = 5) {
  ...
  faker.seed(
    customerId.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0),
  );
...
export async function handleErpTool(name, args) {
  switch (name) {
    ...
    case 'create_order': {
      ...
      return {
        ...
        summary: {
          ...
          totalQuantity: validatedItems.reduce(
    ...
    case 'update_inventory': {
      ...
      const inventory = mockInventory.find(
        (inv) => inv.productId === input.productId,
      );
      if (!inventory) {
        throw new Error(
    ...
    case 'query_inventory': {
      ...
      return {
        ...
        summary: {
          ...
          itemsBelowReorderPoint: enrichedResults.filter((i) => i.needsReorder)
            .length,
          totalStockValue: enrichedResults.reduce(
    ...
    case 'query_orders': {
      ...
      if (input.customerId) {
        const customerOrders = results.filter(
          (o) => o.customerId === input.customerId,
        );
      ...
      return {
        ...
        summary: {
          ...
          totalRevenue: enrichedResults.reduce(
            (sum, o) => sum + o.totalAmount,
            0,
          ),
          averageOrderValue:
            enrichedResults.length > 0
              ? enrichedResults.reduce((sum, o) => sum + o.totalAmount, 0) /
                enrichedResults.length
              : 0,
          statusBreakdown: {
            pending: enrichedResults.filter((o) => o.status === 'pending')
              .length,
            processing: enrichedResults.filter((o) => o.status === 'processing')
              .length,
            shipped: enrichedResults.filter((o) => o.status === 'shipped')
              .length,
            delivered: enrichedResults.filter((o) => o.status === 'delivered')
              .length,
            cancelled: enrichedResults.filter((o) => o.status === 'cancelled')
              .length,
          },
        },
      };
